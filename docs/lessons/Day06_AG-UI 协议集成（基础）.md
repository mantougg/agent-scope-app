...
## 13. 附录 C · 版本与 API 兼容性速查（必看）

> 📌 **进入 Day 6 实操前**先把这五条核对一遍。AG-UI starter 和 `@ag-ui/client` 都还在 0.x / 1.0.x 早期阶段，API 漂得快。下面把所有"hedging 点"集中成一份对照表 + 验证命令，让你 5 分钟把版本和 API 全部锁定，之后正文里就不再嘀咕"以你 jar 为准"了。

### C.1 starter 版本 vs 主包版本

```bash
mvn dependency:tree -Dincludes=io.agentscope:* | grep agentscope
```

**期望输出**：

```
[INFO] +- io.agentscope:agentscope-agui-spring-boot-starter:jar:1.0.12
[INFO] |  +- io.agentscope:agentscope-core:jar:1.0.12   ← 与主包同版本
[INFO] |  \- io.agentscope:agentscope-agui:jar:1.0.12
[INFO] \- io.agentscope:agentscope:jar:1.0.12
```

| 情况 | 处理 |
|------|------|
| starter 与主包同版本 | ✅ 正常，继续 |
| starter 落后（如 1.0.9）反向降级 `agentscope-core` | 用 `<exclusion>` 排掉 starter 带的 `agentscope-core`，让主依赖那份生效 |
| Maven Central 完全没发 starter 这个版本 | 改用 `mvn dependency:resolve` 看实际能拉到最新版，临时改主包版本对齐 |

### C.2 `AguiAgentRegistryCustomizer` lambda 入参的真实类型

starter 把 thread / run 上下文打包成一个对象传给 factory lambda。你不用记类名——**IDE 自动补全就够**：

```java
return registry.registerFactory("analyst", ctx -> {
    // 在这一行末尾敲 .  让 IDE 列出所有方法
    String tid = ctx.threadId();        // 必有
    String rid = ctx.runId();           // 必有
    List<Msg> input = ctx.messages();   // 当前请求的 messages
    // 大概率还有：ctx.eventEmitter() / ctx.metadata()
});
```

如果你想锁实际类名，IDE 鼠标悬停在 `ctx` 上看类型，或者在 starter sources jar 里 grep：

```bash
mvn -q dependency:sources
jar -tf ~/.m2/repository/io/agentscope/agentscope-agui/1.0.12/agentscope-agui-1.0.12-sources.jar \
    | grep -E "Registry|Context|RunInput"
```

### C.3 `@ag-ui/client` 回调名

0.x 期 API 不稳定，常见两种风格——**先看一眼 README 决定怎么写**：

```bash
cd frontend
cat node_modules/@ag-ui/client/README.md | head -50
# 或者看类型定义
cat node_modules/@ag-ui/client/dist/index.d.ts | grep -E "subscribe|on[A-Z]"
```

| 风格 | 形态 | 7.5 节示例用的 |
|------|------|--------------|
| Callbacks 对象 | `agent.subscribe({ onTextMessageContentEvent: ... })` | ✅ |
| EventEmitter | `agent.on('TextMessageContent', cb)` | 备选 |

如果是 EventEmitter 风格，把 7.5 节的 `agent.subscribe({...})` 改成多个 `agent.on(...)` 调用即可，业务逻辑不变。

### C.4 AG-UI 事件载荷字段名

文档里出现的 `e.delta` / `e.messageId` / `e.toolCallId` / `e.snapshot.todos` / `e.delta` (Patch 数组) 等都是 AG-UI 标准字段。万一 starter 实现跟标准有微差，按 DevTools Network 标签的 SSE 帧实际 payload 为准：

```
F12 → Network → 选中 /agui/run 那条 → EventStream tab → 看每帧的 data: JSON
```

### C.5 AS-Java Hook 接口字段（Day 7 §8.3 用）

`ExecutionMetricsHook` 用到 `PreReasoningEvent.getPromptText()` / `PostReasoningEvent.getCompletionTokens()` / `PreActingEvent.getToolCallId()` 等。1.0.12 sources jar grep：

```bash
jar -tf ~/.m2/repository/io/agentscope/agentscope-core/1.0.12/agentscope-core-1.0.12-sources.jar \
    | grep -E "ReasoningEvent|ActingEvent"
# 解压具体看：
jar -xf ~/.m2/repository/io/agentscope/agentscope-core/1.0.12/agentscope-core-1.0.12-sources.jar \
    io/agentscope/core/hook/event/PreReasoningEvent.java
```

字段对不上时按实际改 `ExecutionMetricsHook`，逻辑不动（**记录 LLM/Tool 的开始 ns + 结束 ns 喂给 Micrometer**）。

### C.6 5 分钟自检清单

进入 Day 6 §5 (Phase 2) 实操前，最好把这五项过一遍：

- [ ] C.1：starter 与主包同版本 1.0.12，`mvn dependency:tree` 无降级
- [ ] C.2：IDE 能在 `ctx.` 后看到 `threadId()` / `runId()` 补全
- [ ] C.3：`node_modules/@ag-ui/client/dist/*.d.ts` 里能找到 `subscribe` 或 `on` 方法的签名
- [ ] C.4：先用 §6.1 的 curl 命中 `/agui/run` 一次，确认 SSE 帧里的字段名跟文档对得上
- [ ] C.5：Day 7 §8.3 落地前先 grep 一遍 hook event 类的方法名

5 项都打勾后，正文里所有的 "以你 jar 为准" 都可以视作"已对齐",一路读下去无脑套用。
