    ...
    public AnalysisResult parse(String userRequirement) {
        ...
        for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
            ...
            try {
                ...
                // SchemaValidator.validate 返回 List<ValidationError>，
                // 这里只要中文错误消息回灌给 LLM，转成 List<String>
...
```java
// Day 1 REPL 里读到的一行用变量名 line（不是 input）
...
并在 main 顶部初始化（仓库里实际放在 ScopeApp 的 static 字段，方便后续 Day 4 共用）：
...
## 13. 附录 C · 方案 C：`toolChoice=Specific` 强制单工具

Day 3 主线（方案 A）让 LLM 吐 JSON 文本然后自纠错。还有第三条路 — **方案 C：让 LLM 必须调用一个工具**，工具入参就是 `AnalysisResult` 的字段，由 OpenAI 兼容协议的 `tool_choice` 字段在 API 层强制。

这条路 **不是 Day 4 工具调度的预演**，它跟 Day 4 是正交的：
- Day 4 = toolkit 注册 N 个工具，让 LLM **自己选**调哪个、调几个
- 方案 C = 每次 LLM call **强制** 调指定的那一个工具，参数即结构化输出
...
| `Specific("name")` | **强制** 调指定的那个工具 | 方案 C / 单 schema 产出 |
...
| 场景 | 选谁 | 原因 |
|------|------|------|
| 评测 / 离线评估 / 训练数据生成 | **方案 C** | 需要 100% 确定性结构输出，A 的"自纠错"会引入随机性 |
| schema 合规率要求 ≥99% | **方案 C** | API 层强制 vs prompt 软约束差一个量级 |
| 需要 Agent 多步推理（先想再说） | 方案 A | C 强制马上调工具，LLM 没机会"先思考" |
...
新建 `src/main/java/space/wlshow/scope/tool/SubmitAnalysisTool.java`（注意：今天还不需要真建这个目录，本附录只是示意；Day 4 才正式建 `tool/`）：
...
/**
 * 方案 C 专用"虚拟工具" — 它不真做事，只是 LLM 必须调它，
...
#### C.3.2 `RequirementParserC`（方案 C 版）
...
public class RequirementParserC {

    private final ReActAgent agent;

    public RequirementParserC(ReActAgent agent) { this.agent = agent; }
...
#### C.3.4 REPL 加 `/parseC` 命令

```java
} else if (line.startsWith("/parseC ")) {
    String req = line.substring("/parseC ".length()).trim();
    AnalysisResult r = parserC.parse(req);
    System.out.println("[PARSED-C]\n" + Json.writePretty(r));
    continue;
}
```

### C.4 方案 A vs 方案 C 实测对比
...
- 需要 Agent 在产出前**展示推理过程**（C 一上来就调工具，没思考空间）
- 多 provider 部署，A 是 lowest common denominator

### ✅ 附录 C 验收（可选）

- [x] `/parseC` 跑通 1 个真实需求
- [x] 跟方案 A 同输入对比，看到调用次数差异
- [x] 故意制造一个模糊需求，看 C 是否仍然填 questions（C 通常 questions 会比 A 少，因为它"被强制立刻交付"）
