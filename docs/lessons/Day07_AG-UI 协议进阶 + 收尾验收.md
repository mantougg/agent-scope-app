...
> 📌 `AguiEventEmitter` / `StateDeltaEvent` / `StateSnapshotEvent` 的全限定名以你的 starter（与项目 `${agentscope.version}` 同版本，参见 Day 6 §4.1.1）jar 里实际为准。如果当前版本没有直接暴露 emitter，看 starter 源码里 `AguiRunContext` 或类似抽象，通常会暴露一个 `Sinks.Many<AguiEvent>` 之类。

### 4.3 接入 `AguiAgentConfig`

```java
return registry.registerFactory("analyst", ctx -> {
    // 复用 Day 6 §5.2 已经接好的 FileSession 路径——这里不再走 ThreadContext
    FileSession session = FileSession.loadOrNew(ctx.threadId());

    // 关键：拿到当前 run 的 emitter，构造 bridge 挂到 TodoManager
    AguiStateBridge bridge = new AguiStateBridge(session.todos, ctx.eventEmitter());
    session.todos.addListener(bridge);

    // run 开始时先发一份 snapshot，保证前端跟服务端一致
    bridge.snapshotNow();

    // ... 构造 Agent，沿用 Day 6 的 AgentFactory.buildAnalystWithTools(session.todos, session.memory)
});
```
...
public AguiAgentRegistryCustomizer aguiAgentRegistryCustomizer(
    ...
    return registry -> registry.registerFactory("analyst", ctx -> {
        ...
        if (harnessFactory != null) {
            return harnessFactory.build(ctx.threadId(), session.todos);   // profile=harness 走这条
        }
